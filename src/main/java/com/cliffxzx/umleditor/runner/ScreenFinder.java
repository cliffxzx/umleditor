...
import com.cliffxzx.umleditor.EntryPoint;
...
import com.cliffxzx.umleditor.util.Tuple2;
import com.cliffxzx.umleditor.util.Tuple4;
...
public class ScreenFinder {
    ...
    /**
     * Finds Models, Views, and Controllers annotated with @MVC, and returns them as
     * screen creators.
     *
     * @return the array of screen creators
     * @throws ScreenMissingPartsException the screen is missing a Model, View, or
     *                                     Controller
     * @throws DuplicateScreenException    there is more than one of the same Model,
     *                                     View Or Controller
     */
    @SuppressWarnings("unchecked")
    public Tuple2<ScreenCreator<?, ?, ?>, Boolean>[] find()
            throws ScreenMissingPartsException, DuplicateScreenException {
        ...
        finder.find(MVC.class).forEach((clazz) -> {
            ...
            Tuple4<Class<? extends Model>, Class<? extends View>, Class<? extends Controller>, Boolean> v = mvc
                    .getOrDefault(key, new Tuple4<>(null, null, null, false));

            if (Model.class.isAssignableFrom(clazz)) {
                if (v.first != null) {
                    throw new DuplicateScreenException("Duplicate model " + clazz);
                }
                v = new Tuple4<>((Class<? extends Model>) clazz, v.second, v.third,
                        v.fourth || null != clazz.getAnnotation(EntryPoint.class));
            } else if (View.class.isAssignableFrom(clazz)) {
                if (v.second != null) {
                    throw new DuplicateScreenException("Duplicate view " + clazz);
                }
                v = new Tuple4<>(v.first, (Class<? extends View>) clazz, v.third,
                        v.fourth || null != clazz.getAnnotation(EntryPoint.class));
            } else if (Controller.class.isAssignableFrom(clazz)) {
                if (v.third != null) {
                    throw new DuplicateScreenException("Duplicate controller " + clazz);
                }
                v = new Tuple4<>(v.first, v.second, (Class<? extends Controller>) clazz,
                        v.fourth || null != clazz.getAnnotation(EntryPoint.class));
            } else {
                throw new IllegalArgumentException(
                        "Annotated class " + clazz + " does not extend from Model, View, or Controller");
            }
        ...
        for (Map.Entry<String, Tuple4<Class<? extends Model>, Class<? extends View>, Class<? extends Controller>, Boolean>> entry : mvc
                .entrySet()) {
            ...
            creators.add(new Tuple2<>(new ScreenCreator<>(entry.getValue().first, entry.getValue().second,
                    entry.getValue().third, entry.getKey()), entry.getValue().fourth));
        }
